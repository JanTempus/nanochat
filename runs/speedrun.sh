...
# run base training and eval the model
python -m torch.distributed.run --standalone --nproc_per_node=4 -m scripts.base_train -- --depth=12 --target-param-data-ratio=8 --device-batch-size=32 --fp8 --run=$WANDB_RUN

python -m torch.distributed.run --standalone --nproc_per_node=4 -m scripts.chat_eval -- -i sft_
